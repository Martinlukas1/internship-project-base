...
public class FifteenPercentDiscount {
    ...
    private boolean shouldApply(Receipt receipt) {
        return ifContainsGrainProd(receipt);
    }

    private boolean ifContainsGrainProd(Receipt receipt) {
        int countGrainProd = 0;
        for (ReceiptEntry entry : receipt.entries()) {
            if (entry.product().type().equals(Product.Type.GRAINS)) {
                countGrainProd += entry.quantity();
            }
        }
        return (countGrainProd >= 3);
    }
}
